"""Core k-anonymity set size computation."""
...
class AnonymityEngine:
    ...
    def compute_k_batch(
        ...
        """Compute k for multiple constraint sets."""
    ...
    def compute_delta_k(
        ...
        """Compute the change in k when adding a new constraint. Returns (k_before, k_after, delta_k)."""
    ...
    def get_rare_values(self, qi_name: str, threshold: float = 0.001) -> List[Any]:
        """Get values below the given frequency threshold."""
        if qi_name not in self._value_frequencies:
            return []
...
def compute_k(
    ...
    """Convenience wrapper; for repeated use, instantiate AnonymityEngine directly."""
