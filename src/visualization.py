...
class Visualizer:
    ...
    def plot_turns_to_threshold_distribution(
        ...
        threshold: int = 5,
        title: str = None,
        turns_df: pd.DataFrame = None
    ) -> plt.Figure:
        """Figure 2: Histogram of turns to reach small-cell threshold by model.

        Args:
            all_results: Dict of model_key -> AggregatedResults (original interface).
            threshold: k threshold value.
            title: Figure title.
            turns_df: Pre-built DataFrame with 'model' and 'turns' columns.
                       If provided, all_results is ignored.
        """
        if title is None:
            title = f"Distribution of Steps to Reach k < {threshold}"

        # Build turns_df from all_results if not provided directly
        if turns_df is None:
            if all_results is None:
                raise ValueError("Either all_results or turns_df must be provided")
            turns_data = []
        ...
        for ax, (model_key, model_name) in zip(axes, MODEL_NAMES.items()):
            ...
            # Histogram with density (half-integer bins center bars on integers)
            color = COLORS[model_key]
            bins = [i - 0.5 for i in range(1, 13)]  # [0.5, 1.5, 2.5, ..., 11.5]
            ax.hist(model_turns, bins=bins,
